...
* 在 Note 中填写 MarkLume Deploy

* 勾选 repo 权限（完整的仓库控制），如果需要自动创建仓库，还要勾选 delete_repo？实际上创建仓库需要的权限是 repo 和
  workflow（如果要用 Actions）或者只需要 repo 就足够推送到已有仓库。建议勾选 repo 全部子项。

* 点击最下方 Generate token
